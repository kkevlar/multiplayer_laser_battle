...
__attribute__((warn_unused_result)) bool publicClientShutdown(NetworksHandle* handle)
{
    return packetFillSendHeaderOnlyPacket(handle->net_context->socketNum, FLAG_CLIENT_EXIT_REQUEST);
}
...
{
    uint8_t buf[4096];
    ChatHeader* header = packetFillIncomingUsingDualRecv(context->socketNum, buf, sizeof(buf));
    if (!header)
    {
...
static bool setupHandle(std::string handle, int fd)
{
    ...
    else if (TO_CHAT_FLAG(header->flag) == FLAG_SERVER_HANDLE_PROPOSAL_REJECTION)
    {
