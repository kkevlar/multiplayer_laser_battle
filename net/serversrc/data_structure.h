...
CHECK_RETURN_VAL bool clientMapCheckHandleExistence(const std::string handle);
CHECK_RETURN_VAL PersistentInfo clientMapAddHandle(const std::string handle, int fd);
CHECK_RETURN_VAL bool clientMapDeleteHandle(const std::string handle);
CHECK_RETURN_VAL int clientMapFindByHandle(const std::string handle);
