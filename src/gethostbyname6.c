...
{
    // Puts host IPv6 (or mapped IPV) into the aSockaddr6 struct and return
    // pointer to 16 byte address (NULL on error) Only pulls the first IP
    // address from the list of possible addresses
    ...
    {
        memcpy(((struct sockaddr_in6*)aSockaddr6)->sin6_addr.s6_addr,
               &(((struct sockaddr_in6*)hostInfo->ai_addr)->sin6_addr.s6_addr),
               16);
        memcpy(ipAddress,
               &((struct sockaddr_in6*)aSockaddr6)->sin6_addr.s6_addr,
               16);
