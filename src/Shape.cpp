                     ...
                     unsigned char*(loadimage)(char const*, int*, int*, int*, int))
{
    ...
    for (int i = 0; i < objMaterials.size(); i++)
        if (objMaterials[i].diffuse_texname.size() > 0)
        {
            ...
            if (subdir > 0) filename = filename.substr(subdir + 1);
...
void Shape::init()
{
...

    {
        ...
        glBufferData(
            GL_ELEMENT_ARRAY_BUFFER, eleBuf[i].size() * sizeof(unsigned int), eleBuf[i].data(), GL_STATIC_DRAW);
...
void Shape::draw(const shared_ptr<Program> prog) const
{
...

    {
        ...
        if (textureindex >= 0) glBindTexture(GL_TEXTURE_2D, textureIDs[textureindex]);
